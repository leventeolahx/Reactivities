...
axios.interceptors.response.use(undefined, error => {
    ...
    let wwwAuth = headers['www-authenticate'] && headers['www-authenticate'] as string;
    if (status === 401 && wwwAuth.startsWith('Bearer error="invalid_token", error_description="The token expired')) {        
...
const Profiles = {
    ...
    update: (profile: Partial<IProfile>) => request.put(`/profiles`, profile),
